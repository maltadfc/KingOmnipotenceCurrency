...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    if (!fProofOfStake)
    {
        if(nHeight == 1){
            ...
            CBitcoinAddress addr("KaocrXMVmUBe2kHjSg7B7gULisvx5LRfMB");
    ...
    int64_t nFees = 0;
    {
        ...
        if (!fProofOfStake){
            if(nHeight == 1){
                pblock->vtx[0].vout[0].nValue = Params().GetProofOfWorkReward(nHeight, nFees)-200000000*COIN;
                pblock->vtx[0].vout[1].nValue = 200000000*COIN;
            }else{
