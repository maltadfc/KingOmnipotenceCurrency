...
#else
    const char* pszModule = "KingOmnipotenceCurrency";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KingOmnipotenceCurrency
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KingOmnipotenceCurrency
    // Mac: ~/Library/Application Support/KingOmnipotenceCurrency
    // Unix: ~/.KingOmnipotenceCurrency
    
    
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KingOmnipotenceCurrency";
#else
    fs::path pathRet;
    ...
    if (pszHome == NULL || strlen(pszHome) == 0)
        ...
        return pathRet / "KingOmnipotenceCurrency";
    #else
        // Unix
        return pathRet / ".KingOmnipotenceCurrency";
    #endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "KingOmnipotenceCurrency.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "KingOmnipotenceCurrencyd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong KingOmnipotenceCurrency will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("KingOmnipotenceCurrency"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
